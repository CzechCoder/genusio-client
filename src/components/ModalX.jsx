...
import { Autocomplete, Button, Checkbox, FormControl, FormControlLabel, FormGroup, FormLabel, MenuItem, Select, TextField } from "@mui/material";
...
const CommodalX = ({ selectedKom, setSelectedImg, temata, kategorie }) => {
  ...
  const [newMarketApproved, setMarketApproved] = useState(selectedKom.marketApproved);
...
{/* //-----------------------         */}
          ...
          <FormGroup>
          <FormControlLabel control={<Checkbox color="secondary" value={1} defaultChecked={checkArr1.includes(1) ? 'defaultChecked' : ''} onChange={(e)=>getToolsValue(e)} />} label="Malá reálná setkání do 30 lidí" />
          <FormControlLabel control={<Checkbox color="secondary" value={2} defaultChecked={checkArr1.includes(2) ? 'defaultChecked' : ''} onChange={(e)=>getToolsValue(e)} />} label="Větší akce od 30 do 200 účastníků" />
          <FormControlLabel control={<Checkbox color="secondary" value={3} defaultChecked={checkArr1.includes(3) ? 'defaultChecked' : ''} onChange={(e)=>getToolsValue(e)} />} label="Konference či veletrhy nad 200 účastníků" />              
          <FormControlLabel control={<Checkbox color="secondary" value={4} defaultChecked={checkArr1.includes(4) ? 'defaultChecked' : ''} onChange={(e)=>getToolsValue(e)} />} label="Online meetingy přes ZOOM, Google meet, Microsoft teams, aj" />              
          <FormControlLabel control={<Checkbox color="secondary" value={5} defaultChecked={checkArr1.includes(5) ? 'defaultChecked' : ''} onChange={(e)=>getToolsValue(e)} />} label="Uzavřené diskusní fórum na vlastním webu" />              
          <FormControlLabel control={<Checkbox color="secondary" value={6} defaultChecked={checkArr1.includes(6) ? 'defaultChecked' : ''} onChange={(e)=>getToolsValue(e)} />} label="Facebook skupinu" />              
          <FormControlLabel control={<Checkbox color="secondary" value={7} defaultChecked={checkArr1.includes(7) ? 'defaultChecked' : ''} onChange={(e)=>getToolsValue(e)} />} label="Linkedin skupinu" />              
          <FormControlLabel control={<Checkbox color="secondary" value={8} defaultChecked={checkArr1.includes(8) ? 'defaultChecked' : ''} onChange={(e)=>getToolsValue(e)} />} label="Slack skupinu" />              
          <FormControlLabel control={<Checkbox color="secondary" value={9} defaultChecked={checkArr1.includes(9) ? 'defaultChecked' : ''} onChange={(e)=>getToolsValue(e)} />} label="Discourse" />              
          <FormControlLabel control={<Checkbox color="secondary" value={10} defaultChecked={checkArr1.includes(10) ? 'defaultChecked' : ''} onChange={(e)=>getToolsValue(e)} />} label="Messenger (hromadný chat se členy)" />              
          <FormControlLabel control={<Checkbox color="secondary" value={11} defaultChecked={checkArr1.includes(11) ? 'defaultChecked' : ''} onChange={(e)=>getToolsValue(e)} />} label="Whatsapp (hromadný chat se členy)" />              
          </FormGroup>

          <h5>Jiné platformy <i>(nepovinné)</i></h5>
              <TextField
                size="small"
                variant="standard"
                className="form-control"
          ...
          <h5>Platby od členů.</h5>
            <FormGroup>
            <FormControlLabel control={<Checkbox color="secondary" value={1} defaultChecked={checkArr2.includes(1) ? 'defaultChecked' : ''} onChange={(e)=>getValue(e)} />} label="Nevybírám" />
            <FormControlLabel control={<Checkbox color="secondary" value={2} defaultChecked={checkArr2.includes(2) ? 'defaultChecked' : ''} onChange={(e)=>getValue(e)} />} label="Jednorázové platby (třeba za vstupy na akce)" />
            <FormControlLabel control={<Checkbox color="secondary" value={3} defaultChecked={checkArr2.includes(3) ? 'defaultChecked' : ''} onChange={(e)=>getValue(e)}/>} label="Pravidelná subscriptions či předplatné" />              
            </FormGroup>

            <h5>Má zájem o:</h5>
            <FormControl fullWidth style={{margin: "15px 0px"}} variant="standard">
          <Select
